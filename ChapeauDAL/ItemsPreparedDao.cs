...
namespace ChapeauDAL
{
    ...
    {
        public List<ItemsPrepared> GetAllItemsPrepared() // int menuid maybe pass the id number here to filter, for the having, like having menuid = @this menu id
        {
            // sql query
        ...
        private List<ItemsPrepared> ReadTables(DataTable dataTable)
        {
            string statusString;
            OrderStatus status = new OrderStatus(); //used to convert the status

            List<ItemsPrepared> itemsPrepared = new List<ItemsPrepared>();
            ...
            foreach (DataRow datarow in dataTable.Rows)
            {
                ...
                ItemsPrepared item = new ItemsPrepared()
                {
